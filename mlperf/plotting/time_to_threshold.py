...
class Plot():
    ...
    def do_plot(self, ordered_vars, settings, param_lists, variables, cfg):
        ...
        for entry in Matrix.all_records(settings, param_lists):
            threshold = float(entry.settings.threshold)
            gpu_name = entry.settings.gpu_type
            gpu_name += " x "+ entry.settings.gpu_count + "gpu"
            gpu_name += " x "+ entry.settings.pod_count + "pods"
            exec_times = []
    ...
    def do_plot(self, ordered_vars, settings, param_lists, variables, cfg):
        ...
        for entry in entries:
            def add_plot(an_entry):
                ...
                gpu_name = gpu_full_name = an_entry.settings.gpu_type
                if self.mig_type:
    ...
    def do_plot(self, ordered_vars, settings, param_lists, variables, cfg):
        ...
        for entry in entries:
            def add_plot(an_entry):
                gpu_name = gpu_full_name = an_entry.settings.gpu_type
                if self.mig_type:
                    gpu_name = f"{an_entry.settings.gpu_type} x {an_entry.settings.gpu_count}"
                else:
                    if gpu_name.endswith("_1"): gpu_name = gpu_full_name[:-2]

                    if gpu_name == "full":
                        if self.multi_gpu:
                            gpu_name = f"{an_entry.settings.gpu_count} GPU" + ('s' if int(an_entry.settings.gpu_count) > 1 else '')
                            if self.full_gpu_isolation:
                                gpu_name += f"x {an_entry.settings.pod_count} Pods"
                        else:
                            gpu_name = "8g.40gb (full)"

                if self.multi_gpu:
                    x_value = int(an_entry.settings.pod_count) if self.full_gpu_isolation \
                        else int(an_entry.settings.gpu_count)
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                ...
                range=[0, y_max*(1.05 if self.speed else 1.05)],
            ),
            xaxis=dict(
                range=[0.9, max(x)*1.085],
                title=x_title,
            ),
            title=plot_title, title_x=0.5)
